 ...
 * `next` sends her back to this tool right after the key is saved. */
export function AiKeyBanner({ hasKey, next }: { hasKey: boolean; next?: string }) {
  if (hasKey) return null;
  const href = next ? `/ai/keys?next=${encodeURIComponent(next)}` : "/ai/keys";
  return (
